    ...
    SubShader
    {
        ...
        float3 triplanar(float3 worldPos, float scale, float3 blendAxes, int textureIndex)
        {
            ...
            float3 xProjection = UNITY_SAMPLE_TEX2DARRAY(baseTextures,
                                                         float3(scaledWorldPos.y, scaledWorldPos.z, textureIndex)) *
                blendAxes.x;
            float3 yProjection = UNITY_SAMPLE_TEX2DARRAY(baseTextures,
                                                         float3(scaledWorldPos.x, scaledWorldPos.z, textureIndex)) *
                blendAxes.y;
            float3 zProjection = UNITY_SAMPLE_TEX2DARRAY(baseTextures,
                                                         float3(scaledWorldPos.x, scaledWorldPos.y, textureIndex)) *
                blendAxes.z;
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            for (int i = 0; i < layerCount; i ++)
            {
                ...
                float3 textureColour = triplanar(IN.worldPos, baseTextureScales[i], blendAxes, i) * (1 -
